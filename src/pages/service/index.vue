<template>
    <view class="container">
        ...
        <div class="fu">
            <p @click="getUrl('chaton')"><img src="@/static/img/xiao.png" alt="" />在线客服</p>
        </div>

        <div class="gd">
            <span>常见问题</span>
            <div>
                <p v-for="(item, index) in dataList.cai" :key="index" @click="open(item.con)">{{ Number(index) + 1 }}.{{ item.title }} <img src="@/static/img/youjian.png" alt="" /></p>
                <p v-for="(item, index) in apistradeList" :key="dataList.zhuce" @click="open(item.con)">{{ Number(index) + 1 }}.{{ item.title }} <img src="@/static/img/youjian.png" alt="" /></p>
            </div>
        </div>
    </view>
</template>
